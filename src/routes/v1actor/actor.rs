...
async fn authorize(state: web::Data<AppState>, code: web::Json<String>) -> HttpResponse {
  ...
  match state.exchange_code(&code) {
...
async fn rename(state: web::Data<AppState>, actor_id: web::Path<u64>, name: web::Json<String>) -> HttpResponse {
  if name.is_empty() {
    return HttpResponse::BadRequest().body("Name is required");
  }
  ...
  let mut state = state.write().await;
  match state.rename_actor(*actor_id, name.into_inner()) {
...
// #[delete("/{actor_id}")]
async fn revoke(state: web::Data<AppState>, actor_id: web::Path<u64>) -> HttpResponse {
  let mut state = state.write().await;
  match state.revoke_actor_access(*actor_id) {
    Some(_) => HttpResponse::Ok().finish(),
    None => HttpResponse::BadRequest().body("Invalid actor ID"),
  }
}
...
async fn set_access(state: web::Data<AppState>, actor_id: web::Path<u64>, access: web::Json<bool>) -> HttpResponse {
  let mut state = state.write().await;
  match state.set_actor_access(*actor_id, access.into_inner()) {
    Some(_) => HttpResponse::Ok().finish(),
    None => HttpResponse::BadRequest().body("Invalid actor ID"),
  }
}
